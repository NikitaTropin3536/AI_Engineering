...
def compute_quality_flags(summary: DatasetSummary, missing_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    и добавленные:
      ...
      - дубликаты в 'user_id';
      - неоптимальное хранение бинар. катег. 
    """
    flags: Dict[str, Any] = {}
    ...
    # Наличие константных колонок
    flags["has_constant_columns"] = any(col.unique <= 1 and col.non_null > 0 
                                        for col in summary.columns)
    # пустые не учитываем (это ваще не под определение "константные")

    # Дубликаты в 'user_id'
    user_id_col = next(
        (col for col in summary.columns if col.name == "user_id"), 
        None)
    if user_id_col:
        flags["has_suspicious_id_duplicates"] = user_id_col.unique != summary.n_rows
    else:
        flags["has_suspicious_id_duplicates"] = False

    # Неоптимальное хранение бинарных категорий (2 уникальных, но не числовой тип)
    flags["has_binary_categorical_leak"] = any(
        col.unique == 2 and not col.is_numeric 
        for col in summary.columns
    )
    ...
    # (кастомные, конечно, отчасти)
    if flags["has_constant_columns"]:
        score -= 0.1 # Средний штраф: колонки бесполезны
    
    if flags["has_suspicious_id_duplicates"]:
        score -= 0.15 # Побольше штраф: ошибка целостности данных
        
    if flags["has_binary_categorical_leak"]:
        score -= 0.05 # Низкий штраф: проблема эффективности, а не целостности
