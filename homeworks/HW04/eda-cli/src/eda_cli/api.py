...
# ---------- /quality-flags-from-csv: флаги качества CSV через расширенную EDA-логику ----------
...
@app.post(
    ...
    tags=["quality"],
    summary="Оценка CSV-файла флагами качества с использованием EDA-ядра",
)
async def quality_from_csv(file: UploadFile = File(...)) -> QualityFlagsResponse:
    """
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро
    (summarize_dataset + missing_table + compute_quality_flags)
    и возвращает оценку качества данных в виде полного списка тегов качества.

    Это также связывает S03 (CLI EDA) и S04 (HTTP-сервис).
    """
